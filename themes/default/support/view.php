...
<h2><?php echo htmlspecialchars(Flux::message('ViewSupportHeading')) ?>
	<a title='Refresh this page' href='<?= getURL($params->get('id'), $this->url('support', 'view')) ?>'>
...
<?php if (!is_null($ticket_res)): ?>
	...
	<form action="<?php echo $this->urlWithQs ?>" method="post">
		<input type='hidden' name='ticket_id' value='<?= (int) $ticket_res->id ?>' />
		<table>
			<tr>
	...
	<table class="horizontal-table">
		...
		<tr>
			<td style='text-align:center;'><?= date(Flux::config('DateTimeFormat'), strtotime($ticket_res->datetime_submitted)) ?></td>
			<td style='text-align:center;'><?= (int)$ticket_res->id ?></td>
	...
	<form action="<?php echo $this->urlWithQs ?>#reply_area" method="post" class="generic-form">
		...
		<input type='hidden' name='ticket_id' value='<?= (int) $ticket_res->id ?>' />
		<input type='hidden' name='subscribe' value='<?= (int) $ticket_res->subscribe ?>' />
		<input type='hidden' name='subject' value='<?= $ticket_res->subject ?>' />
		<input type='hidden' name='priority' value='<?= $ticket_res->priority ?>' />
				...
				<tr>
					<th>
						...
						<span style='display:block;float:right;'>
							<?= date(Flux::config('DateTimeFormat'), strtotime($row->datetime_submitted)) ?>
						</span>
					</th>
				</tr>
...
<?php else: ?>
<p class='message'><?= Flux::message('TicketNotExists') ?></p>
<?php endif ?>